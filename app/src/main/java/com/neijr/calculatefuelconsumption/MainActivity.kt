...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (actionBar != null) {
            actionBar.title = "Calculadora de Viagem"
        }
        ...
        fun validateInputValue(textInputEditText: TextInputEditText): Boolean {
            ...
            if (value.isEmpty()) {
                textInputEditText.error = "campo obrigatório"
                return false
            }
            return if (isNumber(value)) {
                if (value.toFloat() <= 0) {
                    textInputEditText.error = "este valor não pode ser zero."
                    return false
                } else {
                    true
                }
            } else {
                textInputEditText.error = "valor inválido"
                false
            }
        }
